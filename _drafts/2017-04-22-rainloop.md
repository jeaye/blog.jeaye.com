...
the critical feedback is more vocal, we are quite pleased with RainLoop in
general.

* The ease up setup should not be understated and is much appreciated. Aside
  from unzipping and setting up Apache, nothing is required. The default config
  is no config and that's both comforting and unsettling.
...
* The support for "smart" multiple identities would've been sorely missed, but
  it behaves exactly as I'd like. For an example, my wife has multiple emails,
  each one using a different name. I can select the default profile to use, for
  her fresh emails which she composes. Aside from that, RainLoop will
  automatically use the profile which matches the To of any email she receives.
  So, she can always just reply and RainLoop will use the correct
  profile/name/and ReplyTo.

* While I would like more complete themes, I think the presence of themes out of
  the box is a nice touch.

### A matter of security
Lastly, but absolutely not least, is RainLoop's GPG support. It was originally
...
encrypt/decrypt properly. As a matter of portability, this is terribly
inconvenient.
...
just stored in the browser.

To make matters worse, *the developers of RainLoop consistently close
GPG-related issues, without comment.* (see
...
stable, at least, RainLoop provides a pretty solid webmail application. Don't
expect to get your security kicks here though.
